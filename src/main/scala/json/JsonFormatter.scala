...
object JsonFormatter:
  ...
  /** Convert a java string into as json string.
    *
    * This function adds quotation marks at beginning and end of the string and
    * escapes special characters like quotation marks, newlines etc.
    */
    ...
    def format(json: JsonValue): String =
      json match
        ...
        case JsonObject(fields) =>
          fields
            .map { (name, value) =>
              s"""${toEscapedJsonString(name)}:${format(value)}"""
            }
            .mkString("{", ",", "}")
        case JsonNull => "null"
  ...
  case class Indent(indent: String) extends JsonFormatter:
    ...
    private def formatWithLevel(
        json: JsonValue,
        level: Int,
        sb: StringBuilder
    ): StringBuilder =
      def formatListlike[T](open: String, formatElem: T => Unit, close: String)(
          l: Seq[T]
      ) =
        sb.append(open)
        sb.append("\n")
      ...
      json match
        ...
        case JsonObject(fields) =>
          formatListlike(
            ...
            "}"
          )(fields.filter { case (_, jsonValue) =>
            jsonValue match {
              case JsonNull => false
              case _        => true
            }
