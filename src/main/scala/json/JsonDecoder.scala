...
extension (json: JsonValue)
  def as[A](using decoder: JsonDecoder[A]): Either[JsonDecoder.Error, A] = {
    decoder.decode(json)
  }
...
object JsonDecoder:
  ...
  given JsonDecoder[BigInt] with
    def decode(json: JsonValue): Either[Error, BigInt] = {
      json match
        ...
        case _ =>
          Left(
            Error(s"cannot convert ${json.getClass.getCanonicalName} to BigInt")
          )
    }
  ...
  given JsonDecoder[Int] with
    def decode(json: JsonValue): Either[Error, Int] = {
      json match
        case JsonInt(i) =>
          if i.isValidInt then Right(i.toInt)
          else Left(Error(s"value doesn't fit into Int: $i"))
        case _ =>
          Left(
            Error(s"cannot convert ${json.getClass.getCanonicalName} to Int")
          )
    }
  ...
  given JsonDecoder[String] with
    def decode(json: JsonValue): Either[Error, String] = {
      json match
        ...
        case _ =>
          Left(
            Error(s"cannot convert ${json.getClass.getCanonicalName} to String")
          )
    }
    ...
    def decode(json: JsonValue): Either[Error, Boolean] = {
      json match
        ...
        case _ =>
          Left(
            Error(
              s"cannot convert ${json.getClass.getCanonicalName} to Boolean"
            )
          )
    }

  given listDecoder[A](using
      elementDecoder: JsonDecoder[A]
  ): JsonDecoder[List[A]] with
    def decode(json: JsonValue): Either[Error, List[A]] = {
      json match
        case JsonArray(elements) =>
          elements.foldRight[Either[Error, List[A]]](Right(List.empty[A])) {
            case (a, acc) =>
              acc.flatMap { l =>
  ...
  /** get the names of the elements of a Product
    *
    * @return
    *   returns a list of all the names of a case class
    */
  ...
  /** get a type class instance for all elements of a product
    *
    * @return
    *   returns a list of JsonEncoders for all fields of a case class
    */
            ...
            json match {
              case JsonObject(fields) =>
                val maybeElements = elemLabels
                  ...
                  .foldRight[Either[Error, List[Any]]](Right(List.empty[Any])) {
                    case (a, acc) =>
                      acc.flatMap { l =>
                        a match
                          case Left(ex)       => Left(ex)
