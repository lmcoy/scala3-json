...
class Tokenizer(str: String) {
  ...
  final def tokenize(
      offset: Int,
      acc: Either[Error, List[Token]],
      line: Int,
      col: Int
  ): Either[Error, List[Token]] = {
    if (offset >= str.length) acc
      ...
      acc match
        case Left(error) => acc
        case Right(tokens) =>
          inline def f(x: Option[Position] => Token) = Right(
            x(Some(Position(line, col))) :: tokens
          )
          inline def g(x: Option[Position] => Token) =
            tokenize(nextOffset, f(x), line, col + 1)
          inline def addPositionToNumber(
              x: Token.IntegerNumber | Token.RationalNumber
          ) =
          ...
          codePoint match
            ...
            case c if Character.isSpaceChar(c) =>
              tokenize(nextOffset, acc, line, col + 1)
            case c
              ...
              maybeNumber match
                case Some(number) =>
                  tokenize(
                    number._1,
                    Right(addPositionToNumber(number._2) :: tokens),
                    line,
                    col + (number._1 - offset)
                  )
            ...
            case '"' =>
              ...
              s match
                ...
                case Right(o) =>
                  tokenize(
                    o._1,
                    Right(o._2.copy(pos = Some(Position(line, col))) :: tokens),
                    line,
                    col + (o._1 - offset)
                  )
            case 't'
                if offset + 4 < str.length && str.substring(
                  offset,
                  offset + 4
                ) == "true" =>
              tokenize(
                offset + 4,
                Right(Token.True(Some(Position(line, col))) :: tokens),
                line,
                col + 4
              )
            case 'f'
                if offset + 5 < str.length && str.substring(
                  offset,
                  offset + 5
                ) == "false" =>
              tokenize(
            ...
            case c =>
              Left(
                Error(
                  s"unexpected char `${new StringBuffer().appendCodePoint(c).toString}`",
                  line,
                  col
                )
              )
  ...
  def number(
      offset: Int
  ): Option[(Int, Token.IntegerNumber | Token.RationalNumber)] = {
    ...
    def f(
        ...
        read: (Int, StringBuilder) => (Int, StringBuilder)
    ): (Int, Boolean) = {
      if (offset < str.length && predicate(str.codePointAt(offset))) {
        ...
        if (!emptyAllowed && s.length == 0) (offset, false)
        else {
          acc.append(s)
          (o, true)
        }
    ...
    val (offFraction, number) =
      f(offUnsigend, true, true, _ != '.', unsignedInteger)

    val (offExp, fraction) =
      f(offFraction, false, true, _ == '.', unsignedInteger)
    val (offFinal, exponent) =
      f(offExp, false, false, Character.toLowerCase(_) == 'e', signedInteger)

    if (!number && !fraction) None
    else if (acc.toString == ".") None
    else if (!fraction && !exponent)
      Some((offFinal, Token.IntegerNumber(acc.toString)))
...
object Tokenizer {
  ...
  case class Error(reason: String, line: Int, col: Int)
      extends Exception(s"error at line $line col $col: $reason")
