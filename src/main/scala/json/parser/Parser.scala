...
object Parser {
  ...
  final def parse(s: String): Either[Error, JsonValue] = {
    ...
    val result = for {
      tokens <- tokenizer
        .tokenize(0, Right(Nil), 0, 0)
        .left
        .map(e => Error(e.reason, e.line, e.col))
  ...
  private def parseElement(tokens: List[Token]): Either[Error, Result] = {
    tokens match
      ...
      case Token.IntegerNumber(value, _) :: ts =>
        Right(Result(JsonInt(value.toInt), ts)) // todo overflow?
      case t :: ts =>
        Left(
          Error(
            s"unexpected token: $t",
            t.line.getOrElse(-1),
            t.col.getOrElse(-1)
          )
        )
  ...
  private def parseObject(
    ...
    tokens match
            ...
            parseElement(ts2) match
              ...
              case Right(r) =>
                ...
                r.unparsed match
                  case Nil =>
                    Left(
                      Error("unexpected EOF, expected `,` or `}`", line, col)
                    )
                  case Token.Comma(_) :: ts =>
                    parseObject(ts, (field, r.jsonValue) :: acc)
                  case Token.RightBrace(_) :: ts =>
                    Right(
                      Result(
                        JsonObject(((field, r.jsonValue) :: acc).reverse),
                        ts
                      )
                    )
                  case token :: ts =>
                    Left(
                      Error(
                        "expected `,` or `}`",
                        token.line.getOrElse(-1),
                        token.col.getOrElse(-1)
                      )
                    )
          case t :: ts2 =>
            Left(
              Error(s"expected `:`", t.line.getOrElse(-1), t.col.getOrElse(-1))
            )
      ...
      case t :: ts =>
        Left(
          Error(
            s"expected string as field name",
            t.line.getOrElse(-1),
            t.col.getOrElse(-1)
          )
        )
            ...
            result match
              ...
              case Right(r) =>
                ...
                r.unparsed match
                  ...
                  case Token.RightBracket(_) :: ts =>
                    Right(Result(JsonArray((r.jsonValue :: acc).reverse), ts))
                  case token :: _ =>
                    Left(
                      Error(
                        "expected , or ]",
